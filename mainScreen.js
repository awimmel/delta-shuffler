...
class MainScreen {
    constructor(screen) {
        ...
        this.playlistTable = new PlaylistTable(this.screen, this.filteredPlaylists, this.searchBar, this.playlistDetailsView)
        
        this.backButton.on('keypress', (char, key) => {
            const activeTable = this.playlistDetailsView.children[0].hidden ? this.playlistDetailsView.children[1] : this.playlistDetailsView.children[0]
            if (key.name === 'enter') {
