...
module.exports = function createMenu(mainScreen, searchBar) {
    ...
    toolbarKeypress(backSong, () => {}, focusFunction(pauseSong), () => {}, () => {
        focusText(searchBar)
    }, () => {})
    toolbarKeypress(pauseSong, focusFunction(backSong), focusFunction(skipSong), () => {}, () => {
        focusText(searchBar)
    }, () => {})
    toolbarKeypress(skipSong, focusFunction(pauseSong), focusFunction(refresh), () => {}, () => {
        focusText(searchBar)
    }, () => {})
    toolbarKeypress(refresh, focusFunction(skipSong), focusFunction(settings), () => {}, () => {
        focusText(searchBar)
    }, () => {
        createRefreshPopover(mainScreen, refresh)
    })
    toolbarKeypress(settings, focusFunction(refresh), focusFunction(close), () => {}, () => {
        focusText(searchBar)
    }, () => {
        createSettingsPopover(mainScreen.screen, settings)
    })
    toolbarKeypress(close, focusFunction(settings), () => {}, () => {}, () => {
        focusText(searchBar)
    }, () => process.exit(0))
