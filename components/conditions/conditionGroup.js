...
class ConditionGroup {
    constructor(screen, buildAlgorithmPopover, top, bottom, topOffset) {
        ...
        this.conditions = [new Condition(0, this.screen, this, this.heightAdjustment, true, false, this.bottom)]
        ...
        if (top != null) {
            ...
            this.joinDropdown = new Dropdown(this.screen, ['AND', 'OR'], this.conditionBox, -2, '100%-10', 5, [])
            
            this.conditions[0].adjustVerticalNavigation(this.joinDropdown.button, this.bottom)
            toolbarKeypress(this.closeButton, focusFunction(this.joinDropdown.button), () => {}, focusFunction(top), focusFunction(this.conditions[0]), () => {
                this.buildAlgorithmPopover.removeConditionGroup(this.topOffset)
            })
            toolbarKeypress(this.joinDropdown.button, () => {}, focusFunction(this.closeButton), focusFunction(top), focusFunction(this.conditions[0]), () => {
    ...
    addCondition() {
        const newCondition = new Condition(this.conditions.length, this.screen, this, (this.conditions.length * 3) + this.heightAdjustment, this.conditions.length !== 2, true, this.bottom)
        ...
        lastCondition.createJoinOperator(secondLastCondition ?? this.joinDropdown, newCondition, joinSiblings)
        ...
        if (secondLastCondition != null) {
            secondLastCondition.adjustVerticalNavigation(this.conditions.at(-3) ?? this.joinDropdown, lastCondition)
        }
    ...
    setTopAndBottom(topElement, bottomElement) {
        if (this.joinDropdown != null && this.closeButton != null) {
            toolbarKeypress(this.joinDropdown.button, () => {}, focusFunction(this.closeButton), focusFunction(topElement), focusFunction(this.conditions[0]), () => {
                this.buildAlgorithmPopover.removeConditionGroup(this.topOffset)
            })
        }

        this.bottom = bottomElement
        this.conditions.at(-1).adjustVerticalNavigation(this.conditions.at(-2) ?? this.joinDropdown?.button, bottomElement)
    }
