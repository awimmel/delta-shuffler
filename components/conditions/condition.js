...
class Condition {
    constructor(index, screen, conditionGroup, topOffset, allowAdd, allowDelete, bottom) {
        ...
        this.bottom = bottom
        this.typeDropdown = new Dropdown(screen, ['Song', 'Artist', 'Album'], conditionGroup.conditionBox, topOffset, 0, 8, [])
        this.operatorDropdown = new Dropdown(screen, ['IN', 'NOT IN'], conditionGroup.conditionBox, topOffset, 9, 8, [])
        ...
        setTypicalKeypresses(this.typeDropdown, this.operatorDropdown, this.valueBox, this.addButton, this.deleteButton, this.joinDropdown, null, this.bottom, this.index, this.conditionGroup)
    }
    ...
    adjustVerticalNavigation(top, bottom) {
        this.bottom = bottom
        setTypicalKeypresses(this.typeDropdown, this.operatorDropdown, this.valueBox, this.addButton, this.deleteButton, this.joinDropdown, top, this.bottom, this.index, this.conditionGroup)
    }
    ...
    createJoinOperator(top, bottom, siblings) {
        this.joinDropdown = new Dropdown(this.screen, ['AND', 'OR'], this.conditionGroup.conditionBox, this.topOffset, '100%-8', 5, siblings)
    ...
    deleteJoinOperator(top, bottom) {
        ...
        if (this.allowDelete) {
            this.deleteButton = createDeleteButton(this.conditionGroup.conditionBox, this.topOffset, this.allowAdd)
        }
...
function setTypicalKeypresses(typeDropdown, operatorDropdown, valueBox, addButton, deleteButton, joinDropdown, top, bottom, index, conditionGroup) {
    toolbarKeypress(typeDropdown.button, () => {}, focusFunction(operatorDropdown.button), veritcalNavigationFunction(top, ConditionElements.TYPE), veritcalNavigationFunction(bottom, ConditionElements.TYPE), () => {
        typeDropdown.open()
    })
    toolbarKeypress(operatorDropdown.button, focusFunction(typeDropdown.button), () => {
        focusText(valueBox)
    }, veritcalNavigationFunction(top, ConditionElements.OPERATOR), veritcalNavigationFunction(bottom, ConditionElements.OPERATOR), () => {
        operatorDropdown.open()
    })
    toolbarKeypress(valueBox, focusFunction(operatorDropdown.button), () => {
    ...
    }, veritcalNavigationFunction(top, ConditionElements.VALUE), veritcalNavigationFunction(bottom, ConditionElements.VALUE), () => {})
    if (addButton != null) {
        toolbarKeypress(addButton, () => {
            focusText(valueBox)
        }, () => {
            if (deleteButton != null) {
                deleteButton.focus()
            }
        }, veritcalNavigationFunction(top, ConditionElements.ADD), veritcalNavigationFunction(bottom, ConditionElements.ADD), () => {
            conditionGroup.addCondition()
        })
    }

    if (deleteButton != null) {
        toolbarKeypress(deleteButton, focusFunction(addButton), () => {}, veritcalNavigationFunction(top, ConditionElements.DELETE), veritcalNavigationFunction(bottom, ConditionElements.DELETE), () => {
            conditionGroup.removeCondition(index)
        })
    }

    if (joinDropdown != null) {
        toolbarKeypress(joinDropdown.button, () => {
            focusText(valueBox)
        }, () => {}, veritcalNavigationFunction(top, ConditionElements.JOIN), veritcalNavigationFunction(bottom, ConditionElements.JOIN), () => {
            joinDropdown.open()
        })
    }
}
