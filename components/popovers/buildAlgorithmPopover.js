...
class BuildAlgorithmPopover {
    constructor (name, screen, backButton, searchBar, playlistId) {
        ...
        this.conditionGroups = [new ConditionGroup(this.screen, this, null, this.addConditionButton, 0)]

        toolbarKeypress(this.conditionGroups[0].conditionBox, () => {}, () => {}, () => {}, focusFunction(this.addConditionButton), () => {})
        setAddConditionNavigation(this)
        setRandomizeCheckboxNavigation(this)
        toolbarKeypress(this.closeBox, () => {}, focusFunction(this.saveBox), focusFunction(this.randomizeCheckbox), () => {}, () => {
        ...
        toolbarKeypress(this.saveBox, focusFunction(this.closeBox), () => {}, focusFunction(this.randomizeCheckbox), () => {}, () => {
            const joinOperator = this.conditionGroups.length > 1 ? ` ${this.conditionGroups[1].joinDropdown.getSelectedItem()} ` : ""
            const conditionString = this.conditionGroups.map(conditionGroup => conditionGroup.toString()).join(joinOperator)
    ...
    removeConditionGroup(topOffset) {
        ...
        if (this.addConditionButton != null) {
            ...
            this.addConditionButton = createAddConditionButton(this.buildAlgBox, this.conditionGroups.length)
            setRandomizeCheckboxNavigation(this)
        }
        setAddConditionNavigation(this)
        ...
        this.conditionGroups.forEach((conditionGroup, index, allGroups) => {
            ...
            const bottomElement = index === allGroups.length - 1 ? this.addConditionButton : allGroups[index + 1].joinDropdown
...
function setAddConditionNavigation(buildAlgPopover) {
    toolbarKeypress(buildAlgPopover.addConditionButton, () => {}, () => {}, () => {
        ...
        const newGroup = new ConditionGroup(buildAlgPopover.screen, buildAlgPopover, prevConditionGroup, bottomElement, (12 * buildAlgPopover.conditionGroups.length) + offsetAdjustment)
        newGroup.focus()
...
function setRandomizeCheckboxNavigation(buildAlgPopover) {
    ...
    toolbarKeypress(buildAlgPopover.randomizeCheckbox, () => {}, () => {}, focusFunction(elementAbove), () => {
